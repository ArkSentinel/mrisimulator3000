...
export function ProtocolsEditor() {
  ...
  const loadProtocols = () => {
    setLoading(true);
    fetch('http://localhost:3000/api/admin/protocols', {
  ...
  const handleAddSequence = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch('http://localhost:3000/api/admin/protocols/sequences', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('mri_token')}` },
        body: JSON.stringify({
  ...
  const handleUpdateSequence = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`http://localhost:3000/api/admin/protocols/sequences/${selectedSequence.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${localStorage.getItem('mri_token')}` },
        body: JSON.stringify({
          ...sequenceForm,
  ...
  const handleDeleteSequence = async (seqId: number) => {
    ...
    try {
      await fetch(`http://localhost:3000/api/admin/protocols/sequences/${seqId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${localStorage.getItem('mri_token')}` }
      });
      if (selectedProtocol) {
  ...
  const handleCopySequence = async (seqId: number) => {
    if (!selectedProtocol) return;
    try {
      const res = await fetch(`http://localhost:3000/api/admin/protocols/sequences/${seqId}/copy?target_protocol_id=${selectedProtocol.id}`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${localStorage.getItem('mri_token')}` }
      });
      if (res.ok) {

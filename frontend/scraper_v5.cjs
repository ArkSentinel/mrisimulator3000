...
async function run() {
  ...
  const browser = await puppeteer.launch({
    headless: true,
    executablePath: '/home/nicolas/.cache/puppeteer/chrome/linux-149.0.7827.22/chrome-linux64/chrome',
  ...
  fs.writeFileSync('./data/protocolos_mri.json', JSON.stringify(resultados, null, 2));
  await browser.close();
  console.log(`\n💾 data/protocolos_mri.json (${resultados.length} regiones)`);
}
